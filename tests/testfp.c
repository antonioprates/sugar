...
long double make(int sgn, int exp, uint64_t high, uint64_t low) {
  u128_t x = {low, (0x0000ffffffffffff & high) |
...
void cmps(void) {
  ...
  for (i = 0; i < 6; i++)
    for (j = 0; j < 6; j++)
      cmp(make(i & 1, i >> 1, 0, 0), make(j & 1, j >> 1, 0, 0));
  ...
  for (i = 0; i < 2; i++)
    for (j = 0; j < 112; j++)
      fmul(make(0, 16383, (uint64_t)1 << i, 0),
           make(0, 16383, j < 64 ? 0 : (uint64_t)1 << (j - 64),
                j < 64 ? (uint64_t)1 << j : 0));
}
...
#else
  printf(
      "This test program is intended for a little-endian architecture\n"
