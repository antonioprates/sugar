...
static void calcaddr(uint32_t* base,
                     int* off,
                     int* sgn,
                     int maxoff,
                     unsigned shift) {
...
static void add_param_plan(struct plan* plan,
                           int cls,
                           int start,
                           int end,
                           SValue* v) {
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   before copy_params). */
static int assign_regs(int nb_args,
                       int float_abi,
                       struct plan* plan,
                       int* todo) {
  int i, size, align;
      ...
      if (op >= TOK_ULT && op <= TOK_GT) {
        ...
        switch (op) {
          ...
          case TOK_LT:
            ...
            break;
          case TOK_LE:
            op = TOK_ULE; /* correct in unordered case only if AC bit in FPSR
                             set */
            break;
          case TOK_EQ:
          case TOK_NE:
