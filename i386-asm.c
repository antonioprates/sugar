/*
 *  i386 specific functions for SUGAR assembler
 *
...
#define O(o)                                                                 \
  ((uint64_t)((((o)&0xff00) == 0x0f00) ? ((((o) >> 8) & ~0xff) | ((o)&0xff)) \
                                       : (o)))
/* This constructs instr_type from opcode, type and group.  */
#define T(o, i, g) \
  ((i) | ((g) << OPC_GROUP_SHIFT) | ((((o)&0xff00) == 0x0f00) ? OPC_0F : 0))
#define DEF_ASM_OP0(name, opcode)
#define DEF_ASM_OP0L(name, opcode, group, instr_type) \
  {TOK_ASM_##name, O(opcode), T(opcode, instr_type, group), 0, {0}},
#define DEF_ASM_OP1(name, opcode, group, instr_type, op0) \
  {TOK_ASM_##name, O(opcode), T(opcode, instr_type, group), 1, {op0}},
#define DEF_ASM_OP2(name, opcode, group, instr_type, op0, op1) \
  {TOK_ASM_##name, O(opcode), T(opcode, instr_type, group), 2, {op0, op1}},
#define DEF_ASM_OP3(name, opcode, group, instr_type, op0, op1, op2) \
  {TOK_ASM_##name, O(opcode), T(opcode, instr_type, group), 3, {op0, op1, op2}},
#ifdef SUGAR_TARGET_X86_64
...
static inline int get_reg_shift(SUGARState* s1) {
  int shift, v;
  v = asm_int_expr(s1);
  switch (v) {
    case 1:
...
static void parse_operand(SUGARState* s1, Operand* op) {
  ExprValue e;
  int reg, indir;
  const char* p;
  ...
  if (tok == '%') {
    ...
    reg_error:
      sugar_error("unknown register %%%s", get_tok_str(tok, &tokc));
    }
    next();
  no_skip:;
  } else if (tok == '$') {
...
static void gen_disp32(ExprValue* pe) {
  ...
  if (esym && esym->st_shndx == cur_text_section->sh_num) {
    /* same section: we can output an absolute value. Note
       that the SUGAR compiler behaves differently here because
...
static void asm_rex(int width64,
                    Operand* ops,
                    int nb_ops,
                    int* op_type,
                    int regi,
  ...
  if (rex) {
    if (saw_high_8bit)
      sugar_error("can't encode register %%%ch when REX prefix is required",
...
static void maybe_print_stats(void) {
  ...
  /* print stats about opcodes */
  {
    ...
    printf("size=%d nb=%d f0=%d f1=%d f2=%d f3=%d\n", (int)sizeof(asm_instrs),
           (int)sizeof(asm_instrs) / (int)sizeof(ASMInstr), freq[0], freq[1],
           freq[2], freq[3]);
  }
}
...
ST_FUNC void asm_opcode(SUGARState* s1, int opcode) {
  ...
  for (;;) {
    ...
    if (nb_ops >= MAX_OPERANDS) {
      sugar_error("incorrect number of operands");
    }
    parse_operand(s1, pop);
    if (tok == ':') {
      if (pop->type != OP_SEG || seg_prefix)
        sugar_error("incorrect prefix");
      seg_prefix = segment_prefixes[pop->reg];
      next();
      parse_operand(s1, pop);
      if (!(pop->type & OP_EA)) {
        sugar_error("segment prefix must be followed by memory reference");
      }
    }
    pop++;
    nb_ops++;
...
again:
  ...
  for (pa = asm_instrs; pa->sym != 0; pa++) {
    ...
    if (pa->opcode == 0xb0 && ops[0].type != OP_IM64 &&
        (ops[1].type & OP_REG) == OP_REG64 && !(pa->instr_type & OPC_0F))
      continue;
#endif
  ...
  if (pa->sym == 0) {
    if (opcode >= TOK_ASM_first && opcode <= TOK_ASM_last) {
      ...
      sugar_error("bad operand with opcode '%s'", get_tok_str(opcode, NULL));
    } else {
      ...
      if (ts->len >= 6 && strchr("wlq", ts->str[ts->len - 1]) &&
          !memcmp(ts->str, "cmov", 4)) {
        opcode = tok_alloc(ts->str, ts->len - 1)->tok;
        goto again;
      }
      sugar_error("unknown opcode '%s'", ts->str);
    }
  }
  ...
  if (s == autosize) {
    ...
    if (s == autosize) {
      if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) &&
        ...
        sugar_error("cannot infer opcode suffix");
    }
  }
  ...
  if (nb_ops == 1 && pa->op_type[0] == OPT_DISP8) {
    ...
    no_short_jump:
      ...
      if (v == 0xeb) /* jmp */
        ...
        sugar_error("invalid displacement");
    }
  }
  ...
  for (;;) {
    ...
    switch (c) {
      ...
      case 'g':
        ...
        break;
      default:
        sugar_error("unknown constraint '%c'", c);
        pr = 0;
    }
    if (pr > priority)
      priority = pr;
  ...
  for (i = 0; i < nb_operands; i++) {
    ...
    if (isnum(*str) || *str == '[') {
      ...
      if ((unsigned)k >= i || i < nb_outputs)
        sugar_error("invalid reference in constraint %d ('%s')", i, str);
      op->ref_index = k;
      if (operands[k].input_index >= 0)
        sugar_error("cannot reference twice the same operand");
  ...
  for (i = 0; i < nb_operands; i++) {
    ...
    if (op->reg >= 0) {
      if (is_reg_allocated(op->reg))
        sugar_error("asm regvar requests register that's taken already");
      reg = op->reg;
      goto reg_found;
    }
  try_next:
    ...
    switch (c) {
      ...
      case '&':
        if (j >= nb_outputs)
          sugar_error("'%c' modifier can only be applied to outputs", c);
        reg_mask = REG_IN_MASK | REG_OUT_MASK;
        goto try_next;
      case 'A':
        /* allocate both eax and edx */
        if (is_reg_allocated(TREG_XAX) || is_reg_allocated(TREG_XDX))
          goto try_next;
        op->is_llong = 1;
        op->reg = TREG_XAX;
      ...
      case 'g':
        ...
        break;
      default:
        sugar_error("asm constraint %d ('%s') could not be satisfied", j,
                    op->constraint);
        break;
    }
  ...
  *pout_reg = -1;
  for (i = 0; i < nb_operands; i++) {
    ...
    if (op->reg >= 0 && (op->vt->r & VT_VALMASK) == VT_LLOCAL &&
      ...
      sugar_error("could not find free output register for reloading");
    reg_found2:
      *pout_reg = reg;
      break;
    }
...
ST_FUNC void subst_asm_operand(CString* add_str, SValue* sv, int modifier) {
  ...
  no_offset:;
    ...
    reg = r & VT_VALMASK;
    if (reg >= VT_CONST)
      sugar_error("internal compiler error");
    ...
    if (modifier == 'b') {
      if (reg >= 4)
        sugar_error("cannot use byte register");
      size = 1;
    } else if (modifier == 'h') {
      if (reg >= 4)
        sugar_error("cannot use byte register");
...
ST_FUNC void asm_gen_code(ASMOperand* operands,
                          int nb_operands,
                          int nb_outputs,
                          int is_output,
...
ST_FUNC void asm_clobber(uint8_t* clobber_regs, const char* str) {
  ...
  if (reg >= TOK_ASM_eax && reg <= TOK_ASM_edi) {
    ...
    sugar_error("invalid clobber register '%s'", str);
  }
  clobber_regs[reg] = 1;
}
