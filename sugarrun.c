...
static void set_pages_executable(SUGARState* s1,
                                 void* ptr,
...
static void run_cdtors(SUGARState* s1,
                       const char* start,
                       const char* end,
                       int argc,
                       char** argv,
...
LIBSUGARAPI int sugar_run(SUGARState* s1, int argc, char** argv) {
  ...
  if ((s1->dflag & 16) &&
      (addr_t)-1 == get_sym_addr(s1, s1->runtime_main, 0, 1))
    return 0;
#ifdef CONFIG_SUGAR_BACKTRACE
  if (s1->do_debug)
  ...
  if (s1->do_debug) {
    ...
    rc->esym_end =
        (ElfW(Sym)*)(symtab_section->data + symtab_section->data_offset);
...
static addr_t rt_printline(rt_context* rc,
                           addr_t wanted_pc,
                           const char* msg,
                           const char* skip) {
  char func_name[128];
  ...
  for (esym = rc->esym_start + 1; esym < rc->esym_end; ++esym) {
    ...
    if (type == STT_FUNC || type == STT_GNU_IFUNC) {
      if (wanted_pc >= esym->st_value &&
        ...
        pstrcpy(func_name, sizeof(func_name), rc->elf_str + esym->st_name);
...
#if 0  // def SIGSTKSZ // this causes signals not to work at all on some (older)
       // linuxes
...
#define SUGARSYM(a) \
  {                 \
    ...
    SUGARSYM(printf) SUGARSYM(fprintf) SUGARSYM(fopen) SUGARSYM(fclose)
#undef SUGARSYM
#endif
        {NULL, NULL},
};
