...
static int add_section(struct macho* mo,
                       struct segment_command_64** _seg,
                       char* name) {
...
static void check_relocs(SUGARState* s1, struct macho* mo) {
  ...
  mo->indirsyms =
      new_section(s1, "LEINDIR", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
      ...
      if (sym->st_shndx == SHN_UNDEF || gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
        ...
        if (!attr->dyn_index) {
          ...
          if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
            ...
            *pi = INDIRECT_SYMBOL_LOCAL;
            ...
            put_elf_reloc(s1->symtab, s1->got, attr->got_offset, R_DATA_PTR,
                          sym_index);
          } else
        ...
        if (for_code) {
          if (attr->plt_offset == -1) {
            ...
            jmp[1] = 0x25;
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset + 2,
...
static int check_symbols(SUGARState* s1, struct macho* mo) {
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
    int elf_index =
        ((struct nlist_64*)mo->symtab->data + sym_index - 1)->n_value;
    ...
    dprintf("%4d (%4d): %09llx %4d %4d %4d %3d %s\n", sym_index, elf_index,
            sym->st_value, type, bind, vis, sym->st_shndx, name);
    if (bind == STB_LOCAL) {
      if (mo->ilocal == -1)
...
static void convert_symbol(SUGARState* s1,
                           struct macho* mo,
                           struct nlist_64* pn) {
...
static void convert_symbols(SUGARState* s1, struct macho* mo) {
  struct nlist_64* pn;
  for_each_elem(mo->symtab, 0, pn, struct nlist_64) convert_symbol(s1, mo, pn);
}
...
static void create_symtab(SUGARState* s1, struct macho* mo) {
  ...
  mo->stubs =
      new_section(s1, "__stubs", SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR);
  mo->stubsym =
      put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
...
static void collect_sections(SUGARState* s1, struct macho* mo) {
    ...
    if (flags & SHF_ALLOC) {
      switch (type) {
        ...
        case SHT_SYMTAB:
          ...
          break;
        case SHT_STRTAB:
          sk = s == stabstr_section ? sk_ro_data : sk_discard;
          break;
        case SHT_RELX:
          sk = sk_discard;
          break;
  ...
  numsec = 0;
  mo->elfsectomacho =
      sugar_mallocz(sizeof(*mo->elfsectomacho) * s1->nb_sections);
  for (sk = sk_unknown; sk < sk_last; sk++) {
    ...
    if (DEBUG_MACHO)
      for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
        ...
        printf(
            "%d section %-16s %-10s %09llx %04x %02d %s,%s,%s\n", sk, s->name,
            type == SHT_PROGBITS
                ? "progbits"
                : type == SHT_NOBITS
                      ? "nobits"
                            ...
                            : type == SHT_STRTAB
                                  ...
                                  : type == SHT_INIT_ARRAY
                                        ...
                                        : type == SHT_FINI_ARRAY
                                              ...
                                              : type == SHT_RELX ? "rel"
                                                                 : "???",
            s->sh_addr, (unsigned)s->data_offset, s->sh_addralign,
            flags & SHF_ALLOC ? "alloc" : "", flags & SHF_WRITE ? "write" : "",
...
ST_FUNC int macho_output_file(SUGARState* s1, const char* filename) {
  ...
  if (!ret) {
    ...
    mo.ep->entryoff =
        get_sym_addr(s1, "main", 1, 1) - get_segment(&mo, 1)->vmaddr;
    if (s1->nb_errors)
      goto do_ret;
...
again:
  ...
  if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
    struct fat_arch* fa = load_data(fd, sizeof(fh), fh.nfat_arch * sizeof(*fa));
    ...
    switch (lc->cmd) {
      ...
      case LC_ID_DYLIB: {
        ...
        dprintf(" ID_DYLIB %d 0x%x 0x%x %s\n", dc->timestamp,
                dc->current_version, dc->compatibility_version, soname);
        break;
      }
  ...
  for (i = iextdef; i < iextdef + nextdef; i++) {
    ...
    dprintf("%5d: %3d %3d 0x%04x 0x%016llx %s\n", i, sym->n_type, sym->n_sect,
            sym->n_desc, sym->n_value, strtab + sym->n_strx);
