...
#include <string.h>
    ...
    !defined(__DragonFly__) && !defined(__OpenBSD__) && !defined(__APPLE__) && \
    !defined(__NetBSD__)
#include <malloc.h>
#endif
...
#endif
#if HAVE_SIGACTION
static int (*sigaction_redir)(int signum,
...
#if defined(__arm__)
DLL_EXPORT void* __bound___aeabi_memcpy(void* dst,
                                        const void* src,
                                        size_t size);
DLL_EXPORT void* __bound___aeabi_memmove(void* dst,
                                         const void* src,
                                         size_t size);
DLL_EXPORT void* __bound___aeabi_memmove4(void* dst,
                                          const void* src,
                                          size_t size);
DLL_EXPORT void* __bound___aeabi_memmove8(void* dst,
                                          const void* src,
                                          size_t size);
...
DLL_EXPORT void* __bound_memalign(size_t size,
                                  size_t align,
                                  const void* caller);
...
#define bound_warning(...) sugar_backtrace("^bcheck.c^BCHECK: " __VA_ARGS__)
...
static void bound_not_found_warning(const char* file,
                                    const char* function,
                                    void* ptr) {
...
void __bound_init(size_t* p, int mode) {
...
#if MALLOC_REDIR
  {
    ...
    *(void**)(&pthread_create_redir) = dlsym(addr, "pthread_create");
    dprintf(stderr, "%s, %s(): pthread_create_redir %p\n", __FILE__,
            __FUNCTION__, pthread_create_redir);
...
#ifdef __linux__
  {
    ...
    unsigned long start;
    unsigned long end;
    unsigned long ad = (unsigned long)__builtin_return_address(0);
    char line[1000];
    ...
    if (fp) {
      while (fgets(line, sizeof(line), fp)) {
        if (sscanf(line, "%lx-%lx", &start, &end) == 2 && ad >= start &&
            ad < end) {
          found = 1;
          break;
        }
...
#if HAVE_CTYPE
#ifdef __APPLE__
  tree = splay_insert((size_t)&_DefaultRuneLocale, sizeof(_DefaultRuneLocale),
                      tree);
#else
  /* XXX: Does not work if locale is changed */
  tree = splay_insert((size_t)__ctype_b_loc(), sizeof(unsigned short*), tree);
  tree = splay_insert((size_t)(*__ctype_b_loc() - 128),
                      384 * sizeof(unsigned short), tree);
  tree = splay_insert((size_t)__ctype_tolower_loc(), sizeof(__int32_t*), tree);
  tree = splay_insert((size_t)(*__ctype_tolower_loc() - 128),
                      384 * sizeof(__int32_t), tree);
  tree = splay_insert((size_t)__ctype_toupper_loc(), sizeof(__int32_t*), tree);
...
void __attribute__((destructor)) __bound_exit(void) {
  int i;
  static const char* const alloc_type[] = {"",        "malloc",   "calloc",
                                           "realloc", "memalign", "strdup"};
    ...
    while (tree) {
      if (print_heap && tree->type != 0)
        fprintf(stderr, "%s, %s(): %s found size %lu\n", __FILE__, __FUNCTION__,
                alloc_type[tree->type], (unsigned long)tree->size);
...
int pthread_create(pthread_t* thread,
...
#if HAVE_TLS_FUNC
  {
    ...
    data = (bound_thread_create_type*)BOUND_MALLOC(
        sizeof(bound_thread_create_type));
...
bound_sig signal(int signum, bound_sig handler) {
  ...
  dprintf(stderr, "%s, %s() %d %p\n", __FILE__, __FUNCTION__, signum, handler);
...
int sigaction(int signum,
  ...
  dprintf(stderr, "%s, %s() %d %p %p\n", __FILE__, __FUNCTION__, signum, act,
          oldact);
  if (act) {
    nact = *act;
...
static void __bound_check(const void* p, size_t size, const char* function) {
  if (size != 0 && __bound_ptr_add((void*)p, size) == INVALID_POINTER) {
    bound_error("invalid pointer %p, size 0x%lx in %s", p, (unsigned long)size,
                function);
  }
}

static int check_overlap(const void* p1,
                         size_t n1,
                         const void* p2,
                         size_t n2,
                         const char* function) {
  ...
  if (NO_CHECKING_GET() == 0 && n1 != 0 && n2 != 0 &&
    ...
    bound_error("overlapping regions %p(0x%lx), %p(0x%lx) in %s", p1,
                (unsigned long)n1, p2, (unsigned long)n2, function);
...
#define compare(start, tstart, tsize) \
  (start < tstart ? -1 : start >= tstart + tsize ? 1 : 0)
...
#define compare_end(start, tend) (start < tend ? -1 : start > tend ? 1 : 0)
...
#define compare_destroy(start, tstart) \
  (start < tstart ? -1 : start > tstart ? 1 : 0)
