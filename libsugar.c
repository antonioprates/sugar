...
static void sugar_split_path(SUGARState* s,
                             void* p_ary,
                             int* p_nb_ary,
                             const char* in) {
  const char* p;
...
static void strcat_vprintf(char* buf,
                           int buf_size,
                           const char* fmt,
                           va_list ap) {
  int len;
...
static void error1(int mode, const char* fmt, va_list ap) {
  ...
  if (f) {
    ...
    strcat_printf(buf, sizeof(buf), "%s:%d: ", f->filename,
                  f->line_num - !!(tok_flags & TOK_FLAG_BOL));
...
LIBSUGARAPI void sugar_set_error_func(SUGARState* s,
                                      void* error_opaque,
                                      SUGARErrorFunc error_func) {
...
static int sugar_compile(SUGARState* s1,
                         int filetype,
                         const char* str,
                         int fd) {
...
LIBSUGARAPI void sugar_define_symbol(SUGARState* s1,
                                     const char* sym,
                                     const char* value) {
  const char* eq;
...
#undef gnu_ext
  ...
  /* used by _fd_def.h */
  sugar_define_symbol(s, "__builtin_bzero(p, ignored_size)",
                      "bzero(p, sizeof(*(p)))");
  ...
  sugar_define_symbol(s, "__builtin_offsetof(type,field)",
...
static int sugar_add_library_internal(SUGARState* s,
                                      const char* fmt,
                                      const char* filename,
                                      int flags,
                                      char** paths,
...
ST_FUNC int sugar_add_crt(SUGARState* s1, const char* filename) {
  if (-1 == sugar_add_library_internal(s1, "%s/%s", filename, 0, s1->crt_paths,
                                       s1->nb_crt_paths))
    sugar_error_noabort("file '%s' not found", filename);
  return 0;
}
...
LIBSUGARAPI int sugar_add_library(SUGARState* s, const char* libraryname) {
  ...
  while (*pp) {
    if (0 == sugar_add_library_internal(s, *pp, libraryname, flags,
                                        s->library_paths, s->nb_library_paths))
      return 0;
    ++pp;
  }
...
LIBSUGARAPI int sugar_add_symbol(SUGARState* s1,
                                 const char* name,
                                 const void* val) {
#ifdef SUGAR_TARGET_PE
...
ST_FUNC int set_flag(SUGARState* s, const FlagDef* flags, const char* name) {
  ...
  for (ret = -1, p = flags; p->name; ++p) {
    ...
    if (p->offset) {
      *((unsigned char*)s + p->offset) = p->flags & FD_INVERT ? !value : value;
      if (ret)
        return 0;
    } else {
      ret = 0;
...
static void args_parser_add_file(SUGARState* s,
                                 const char* filename,
                                 int filetype) {
...
PUB_FUNC int sugar_parse_args(SUGARState* s,
  ...
  reparse:
    ...
    switch (popt->index) {
      ...
      set_output_type:
        if (s->output_type)
          sugar_warning("-%s: overriding compiler action already specified",
                        popt->name);
        s->output_type = x;
        break;
      case SUGAR_OPTION_d:
