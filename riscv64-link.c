...
#define EM_SUGAR_TARGET EM_RISCV
...
ST_FUNC void relocate_plt(SUGARState* s1) {
  ...
  if (p < p_end) {
    ...
    if ((off + ((uint32_t)1 << 20)) >> 21)
      sugar_error("Failed relocating PLT (off=0x%lx, got=0x%lx, plt=0x%lx)",
                  off, got, plt);
    ...
    while (p < p_end) {
      ...
      if ((off + ((uint32_t)1 << 20)) >> 21)
        sugar_error("Failed relocating PLT (off=0x%lx, addr=0x%lx, pc=0x%lx)",
                    off, addr, pc);
  ...
  switch (type) {
    ...
    case R_RISCV_BRANCH:
      ...
      if ((off64 + (1 << 12)) & ~(uint64_t)0x1ffe)
        sugar_error(
            "R_RISCV_BRANCH relocation failed"
            " (val=%lx, addr=%lx)",
            val, addr);
      off32 = off64 >> 1;
    ...
    case R_RISCV_PCREL_HI20:
      ...
      if ((off64 + ((uint64_t)1 << 20)) >> 21)
        sugar_error(
            "R_RISCV_PCREL_HI20 relocation failed: off=%lx cond=%lx sym=%s",
    ...
    case R_RISCV_GOT_HI20:
      ...
      off64 = (int64_t)(val - addr + 0x800) >> 12;
      if ((off64 + ((uint64_t)1 << 20)) >> 21)
        sugar_error("R_RISCV_GOT_HI20 relocation failed");
      last_hi.addr = addr;
      last_hi.val = val;
      write32le(ptr, (read32le(ptr) & 0xfff) | ((off64 & 0xfffff) << 12));
      return;
...
#endif
      if (val != last_hi.addr)
        sugar_error("unsupported hi/lo pcrel reloc scheme");
      val = last_hi.val;
      addr = last_hi.addr;
      write32le(ptr,
                (read32le(ptr) & 0xfffff) | (((val - addr) & 0xfff) << 20));
      return;
    case R_RISCV_PCREL_LO12_S:
      if (val != last_hi.addr)
        sugar_error("unsupported hi/lo pcrel reloc scheme");
    ...
    case R_RISCV_RVC_BRANCH:
      ...
      if ((off64 + (1 << 8)) & ~(uint64_t)0x1fe)
        sugar_error(
            "R_RISCV_RVC_BRANCH relocation failed"
            " (val=%lx, addr=%lx)",
            val, addr);
      off32 = off64;
    ...
    case R_RISCV_RVC_JUMP:
      ...
      off32 = off64;
      write16le(ptr,
                (read16le(ptr) & 0xe003) | (((off32 >> 5) & 1) << 2) |
    ...
    case R_RISCV_32:
      if (s1->output_type == SUGAR_OUTPUT_DLL) {
        /* XXX: this logic may depend on SUGAR's codegen
           now SUGAR uses R_RISCV_RELATIVE even for a 64bit pointer */
