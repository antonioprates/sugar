...
static int sugar_assemble_internal(SUGARState* s1,
                                   int do_preprocess,
                                   int global);
static Sym* asm_new_label(SUGARState* s1, int label, int is_local);
static Sym* asm_new_label1(SUGARState* s1,
                           int label,
                           int is_local,
                           int sh_num,
                           int value);
...
static Sym* asm_section_sym(SUGARState* s1, Section* sec) {
  char buf[100];
  int label = tok_alloc(buf, snprintf(buf, sizeof buf, "L.%s", sec->name))->tok;
...
static void sugar_assemble_inline(SUGARState* s1,
                                  char* str,
                                  int len,
                                  int global) {
...
/* find a constraint by its number or id (gcc 3 extended
   ...
   constraint */
ST_FUNC int find_constraint(ASMOperand* operands,
                            int nb_operands,
                            const char* name,
                            const char** pp) {
  int index;
...
static void parse_asm_operands(ASMOperand* operands,
                               int* nb_operands_ptr,
                               int is_output) {
  ASMOperand* op;
  int nb_operands;
