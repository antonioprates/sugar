...
static void* tal_realloc_impl(TinyAlloc** pal,
                              void* p,
...
static TokenSym* tok_alloc_new(TokenSym** pts, const char* str, int len) {
  ...
  if ((i % TOK_ALLOC_INCR) == 0) {
    ptable =
        sugar_realloc(table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym*));
    table_ident = ptable;
  }
...
ST_FUNC void label_pop(Sym** ptop, Sym* slast, int keep) {
  ...
  for (s = *ptop; s != slast; s = s1) {
    ...
    if (s->r == LABEL_DECLARED) {
      sugar_warning("label '%s' declared but not used",
                    get_tok_str(s->v, NULL));
    } else if (s->r == LABEL_FORWARD) {
      sugar_error("label '%s' used but not defined", get_tok_str(s->v, NULL));
    } else {
      if (s->c) {
...
static CachedInclude* search_cached_include(SUGARState* s1,
                                            const char* filename,
                                            int add) {
  ...
  switch (tok) {
    ...
    default:
      ...
      if (tok == '!' && is_bof)
        ...
        goto ignore;
      sugar_warning("Ignoring unknown preprocessing directive #%s",
...
static void parse_escape_string(CString* outstr,
                                const uint8_t* buf,
                                int is_long) {
  int c, n;
  const uint8_t* p;
...
#endif
            search_cached_include(s1, file->filename, 1)->ifndef_macro =
...
static int* macro_arg_subst(Sym** nested_list,
                            const int* macro_str,
                            Sym* args) {
  int t, t0, t1, spc;
  const int* st;
  ...
  while (1) {
    ...
    if (t == '#') {
      ...
      if (s) {
        ...
        while (*st >= 0) {
          TOK_GET(&t, &st, &cval);
          if (t != TOK_PLCHLDR && t != TOK_NOSUBST &&
              0 == check_space(t, &spc)) {
...
static int macro_subst_tok(TokenString* tok_str, Sym** nested_list, Sym* s) {
  ...
  add_cstr1:
    ...
    if (s->type.t == MACRO_FUNC) {
      ...
      if (sa) {
        sugar_error("macro '%s' used with too few args", get_tok_str(s->v, 0));
      }
...
static void sugar_predefs(CString* cstr) {
  ...
  //"#include <sugar_predefs.h>\n"
      ...
      "#define __builtin_va_arg(ap,t) "
      "((sizeof(t)>8||(sizeof(t)&(sizeof(t)-1)))?**(t**)((ap+=8)-8):*(t*)((ap+="
      "8)-8))\n"
#endif
#elif defined SUGAR_TARGET_ARM
      "typedef char*__builtin_va_list;\n"
      "#define _sugar_alignof(type) ((int)&((struct{char c;type x;}*)0)->x)\n"
      "#define _sugar_align(addr,type) "
      "(((unsigned)addr+_sugar_alignof(type)-1)&~(_sugar_alignof(type)-1))\n"
      "#define __builtin_va_start(ap,last) "
      "(ap=((char*)&(last))+((sizeof(last)+3)&~3))\n"
      "#define __builtin_va_arg(ap,type) "
      "(ap=(void*)((_sugar_align(ap,type)+sizeof(type)+3)&~3),*(type*)(ap-(("
      "sizeof(type)+3)&~3)))\n"
      ...
      "#define _sugar_align(addr,type) (((unsigned "
      "long)addr+__alignof__(type)-1)&-(__alignof__(type)))\n"
      "#define __builtin_va_arg(ap,type) "
      ...
      "#define __BUILTINBC(ret,name,params) ret __builtin_##name params "
      "__attribute__((alias(\"__bound_\"#name)));\n"
      "#define __BOUND(ret,name,params) ret name params "
      "__attribute__((alias(\"__bound_\"#name)));\n"
      "#else\n"
      "#define __BUILTINBC(ret,name,params) ret __builtin_##name params "
      ...
      "__BUILTINBC(ret,name,params)__BOUND(ret,name,params)\n"
      "#define __BUILTIN(ret,name,params) ret __builtin_##name params "
      "__attribute__((alias(\"\"#name)));\n"
