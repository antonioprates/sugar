...
void relocate(SUGARState *s1, ElfW_Rel *rel, int type, unsigned char *ptr,
              addr_t addr, addr_t val)
{
    ...
    case R_RISCV_BRANCH:
        ...
        if ((off64 + (1 << 12)) & ~(uint64_t)0x1ffe)
          sugar_error("R_RISCV_BRANCH relocation failed"
    ...
    case R_RISCV_JAL:
        ...
        if ((off64 + (1 << 21)) & ~(((uint64_t)1 << 22) - 2))
          sugar_error("R_RISCV_JAL relocation failed"
                    " (val=%lx, addr=%lx)", val, addr);
        off32 = off64;
    ...
    case R_RISCV_PCREL_HI20:
        ...
        if ((off64 + ((uint64_t)1 << 20)) >> 21)
          sugar_error("R_RISCV_PCREL_HI20 relocation failed: off=%lx cond=%lx sym=%s",
    ...
    case R_RISCV_RVC_BRANCH:
        ...
        if ((off64 + (1 << 8)) & ~(uint64_t)0x1fe)
          sugar_error("R_RISCV_RVC_BRANCH relocation failed"
                    " (val=%lx, addr=%lx)", val, addr);
        off32 = off64;
