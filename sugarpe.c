/*
 *  SUGARPE.C - PE file output for the Sugar C Compiler
 *
...
#elif defined SUGAR_TARGET_I386
#define ADDR3264 DWORD
#define PE_IMAGE_REL IMAGE_REL_BASED_HIGHLOW
...
static const char* pe_export_name(SUGARState* s1, ElfW(Sym) * sym) {
...
static int pe_find_import(SUGARState* s1, ElfW(Sym) * sym) {
  char buffer[200];
  const char *s, *p;
  int sym_index = 0, n = 0;
  int a, err = 0;
  ...
  do {
    ...
    // printf("find (%d) %d %s\n", n, sym_index, s);
    if (sym_index && ELFW(ST_TYPE)(sym->st_info) == STT_OBJECT &&
        0 == (sym->st_other & ST_PE_IMPORT) && 0 == a)
      err = -1, sym_index = 0;
...
static void pe_set_datadir(struct pe_header* hdr,
                           int dir,
                           DWORD addr,
                           DWORD size) {
...
#define CHARACTERISTICS_DLL 0x230F
  ...
  if (NULL == pf.op) {
    sugar_error_noabort("could not write '%s': %s", pe->filename,
                        strerror(errno));
    return -1;
  }

  pe->sizeofheaders =
      pe_file_align(pe, sizeof(struct pe_header) +
  ...
  for (i = 0; i < pe->sec_count; ++i) {
    ...
    if (2 == pe->s1->verbose)
      printf("%6x %6x %6x  %s\n", (unsigned)addr, (unsigned)file_offset,
             (unsigned)size, sh_name);
    ...
    if (pe->imp_size) {
      pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_IMPORT, pe->imp_offs,
                     pe->imp_size);
      pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_IAT, pe->iat_offs,
                     pe->iat_size);
    }
    if (pe->exp_size) {
      pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_EXPORT, pe->exp_offs,
                     pe->exp_size);
    }
...
static void pe_build_imports(struct pe_info* pe) {
  ...
  for (i = 0; i < pe->imp_count; ++i) {
    ...
    if (dllindex)
      name = sugar_basename((dllref = pe->s1->loaded_dlls[dllindex - 1])->name);
    else
      name = "", dllref = NULL;
    ...
    for (k = 0, n = p->sym_count; k <= n; ++k) {
      if (k < n) {
        ...
        ElfW(Sym)* imp_sym =
            (ElfW(Sym)*)pe->s1->dynsymtab_section->data + sym_index;
        ElfW(Sym)* org_sym = (ElfW(Sym)*)symtab_section->data + iat_index;
        const char* name =
            (char*)pe->s1->dynsymtab_section->link->data + imp_sym->st_name;
        int ordinal;
        ...
        if (dllref)
          ...
          ordinal = 0,
          v = imp_sym->st_value; /* address from sugar_add_symbol() */
        ...
        if (pe->type == PE_RUN) {
          ...
          if (!v)
            sugar_error_noabort("could not resolve symbol '%s'", name);
        } else
#endif
            if (ordinal) {
...
static void pe_build_exports(struct pe_info* pe) {
  ...
  pe_align_section(pe->thunk, 16);
  dllname = sugar_basename(pe->filename);
  ...
  strcpy(sugar_fileextension(buf), ".def");
  op = fopen(buf, "wb");
  if (NULL == op) {
    sugar_error_noabort("could not create '%s': %s", buf, strerror(errno));
  } else {
  ...
  for (ord = 0; ord < sym_count; ++ord) {
    ...
    put_elf_reloc(symtab_section, pe->thunk, func_o, R_XXX_RELATIVE, sym_index);
...
static int pe_assign_addresses(struct pe_info* pe) {
  ...
  section_order = sugar_malloc(pe->s1->nb_sections * sizeof(int));
  ...
  add_section:
    ...
    // printf("%08x %05x %08x %s\n", si->sh_addr, si->sh_size, si->pe_flags,
    // s->name);
  }
  sugar_free(section_order);
#if 0
    for (i = 1; i < pe->s1->nb_sections; ++i) {
...
static int pe_isafunc(SUGARState* s1, int sym_index) {
    ...
    if (sym->st_shndx == SHN_UNDEF) {
      ...
      if (type == STT_FUNC) {
        ...
        if (offset) {
          ...
          p = section_ptr_add(text_section,
          ...
          *(DWORD*)(p + 1) = (DWORD)-4;
          ...
          is->iat_index = put_elf_sym(symtab_section, 0, sizeof(DWORD),
                                      ...
                                      SHN_UNDEF, buffer);
#ifdef SUGAR_TARGET_ARM
          put_elf_reloc(symtab_section, text_section, offset + 8,
                        R_XXX_THUNKFIX,
                        is->iat_index);  // offset to IAT position
#else
          put_elf_reloc(symtab_section, text_section, offset + 2,
                        R_XXX_THUNKFIX, is->iat_index);
#endif
          is->thk_offset = offset;
        }

        /* sugar_realloc might have altered sym's address */
        sym = (ElfW(Sym)*)symtab_section->data + sym_index;
...
static void pe_print_section(FILE* f, Section* s) {
  ...
  if (s->sh_type == SHT_SYMTAB || s->sh_type == SHT_RELX) {
    const char* fields1[] = {"name", "value", "size",  "bind",
                             "type", "other", "shndx", NULL};

    const char* fields2[] = {"offs", "type", "symb", NULL};
  ...
  for (i = 0; i < l;) {
    ...
    if (s->sh_type == SHT_SYMTAB) {
      ...
      fprintf(f, "  %04X  %04X  %04X   %02X    %02X    %02X   %04X  \"%s\"",
              (unsigned)sym->st_name, (unsigned)sym->st_value,
              (unsigned)sym->st_size, (unsigned)ELFW(ST_BIND)(sym->st_info),
              (unsigned)ELFW(ST_TYPE)(sym->st_info), (unsigned)sym->st_other,
              (unsigned)sym->st_shndx, name);

    } else if (s->sh_type == SHT_RELX) {
      ElfW_Rel* rel = (ElfW_Rel*)(p + i);
      ElfW(Sym)* sym = (ElfW(Sym)*)s->link->data + ELFW(R_SYM)(rel->r_info);
      const char* name = s->link->link->data + sym->st_name;
      fprintf(f, "  %04X   %02X   %04X  \"%s\"", (unsigned)rel->r_offset,
              (unsigned)ELFW(R_TYPE)(rel->r_info),
              (unsigned)ELFW(R_SYM)(rel->r_info), name);
    } else {
      fprintf(f, "   ");
      for (n = 0; n < m; ++n) {
        if (n + i < l) {
...
static void pe_print_sections(SUGARState* s1, const char* fname) {
  Section* s;
  FILE* f;
  int i;
  f = fopen(fname, "w");
...
ST_FUNC int pe_putimport(SUGARState* s1,
                         int dllindex,
                         const char* name,
                         addr_t value) {
  return set_elf_sym(s1->dynsymtab_section, value, dllindex, /* st_size */
                     ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
                     value ? SHN_ABS : SHN_UNDEF, name);
}

static int add_dllref(SUGARState* s1, const char* dllname) {
  DLLReference* dllref;
  int i;
  for (i = 0; i < s1->nb_loaded_dlls; ++i)
    if (0 == strcmp(s1->loaded_dlls[i]->name, dllname))
      return i + 1;
  dllref = sugar_mallocz(sizeof(DLLReference) + strlen(dllname));
...
PUB_FUNC int sugar_get_dllexports(const char* filename, char** pp) {
  ...
  for (i = 0; i < ih.NumberOfSections; ++i) {
    if (!read_mem(fd, sec_hdroffset + i * sizeof ish, &ish, sizeof ish))
      goto the_end;
    // printf("vaddr: %08x\n", ish.VirtualAddress);
    if (addr >= ish.VirtualAddress &&
        addr < ish.VirtualAddress + ish.SizeOfRawData)
      goto found;
  }
  goto the_end_0;
...
found:
  ...
  for (i = 0; i < ied.NumberOfNames; ++i) {
    ...
    for (l = 0;;) {
      if (n + 1 >= n0)
        p = sugar_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(SUGARState* s1, int fd) {
  ...
  sym_index =
      put_elf_sym(symtab_section, 0, 0, 0, 0, rsrc_section->sh_num, ".rsrc");
  for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i) {
    ...
    if (rel.type != RSRC_RELTYPE)
      goto quit;
    put_elf_reloc(symtab_section, rsrc_section, rel.offset, R_XXX_RELATIVE,
                  sym_index);
    offs += sizeof rel;
  }
  ret = 0;
...
/* ------------------------------------------------------------- */
static int pe_load_def(SUGARState* s1, int fd) {
...
/* ------------------------------------------------------------- */
static int pe_load_dll(SUGARState* s1, const char* filename) {
  char *p, *q;
  int index, ret;

  ret = sugar_get_dllexports(filename, &p);
  if (ret) {
    return -1;
  } else if (p) {
...
ST_FUNC int pe_load_file(struct SUGARState* s1, const char* filename, int fd) {
  int ret = -1;
  char buf[10];
  if (0 == strcmp(sugar_fileextension(filename), ".def"))
...
/* ------------------------------------------------------------- */
#ifdef SUGAR_TARGET_X86_64
static unsigned pe_add_uwwind_info(SUGARState* s1) {
  if (NULL == s1->uw_pdata) {
  ...
  if (0 == s1->uw_offs) {
    ...
    static const unsigned char uw_info[] = {
        ...
        0x05,  // UBYTE: 4 Frame Register (rbp), UBYTE: 4 Frame Register offset
               // (scaled)
...
static void sugar_add_support(SUGARState* s1, const char* filename) {
  if (sugar_add_dll(s1, filename, 0) < 0)
    sugar_error_noabort("%s not found", filename);
}
...
static void pe_add_runtime(SUGARState* s1, struct pe_info* pe) {
  ...
  /* grab the startup code from libsugar1.a */
#ifdef SUGAR_IS_NATIVE
  if (SUGAR_OUTPUT_MEMORY != s1->output_type || s1->runtime_main)
#endif
  ...
  /* need this for 'sugarelf.c:relocate_section()' */
  if (SUGAR_OUTPUT_DLL == s1->output_type)
...
static void pe_set_options(SUGARState* s1, struct pe_info* pe) {
  if (PE_DLL == pe->type) {
...
ST_FUNC int pe_output_file(SUGARState* s1, const char* filename) {
  ...
  if (ret)
    ...
    pe.start_addr =
        (DWORD)(get_sym_addr(s1, pe.start_symbol, 1, 1) - pe.imagebase);
    if (s1->nb_errors)
      ret = -1;
    else
      ret = pe_write(&pe);
...
#ifdef PE_PRINT_SECTIONS
  pe_print_sections(s1, "sugar.log");
#endif
  return ret;
}
