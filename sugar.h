...
#ifdef _MSC_VER
#pragma warning(disable : 4244)  // conversion from 'uint64_t' to 'int',
...
#define CONFIG_SUGAR_LIBPATHS                                         \
  ...
  ":" ALSO_TRIPLET(CONFIG_SYSROOT "/" CONFIG_LDDIR) ":" ALSO_TRIPLET( \
...
#define CONFIG_SUGAR_ELFINTERP \
  "/lib/ld-uClibc.so.0" /* is there a uClibc for x86_64 ? */
...
struct SymAttr {
  unsigned short aligned : 5, /* alignment as log2+1 (0 == unspecified) */
      ...
      dllimport : 1, addrtaken : 1, xxxx : 3; /* not used */
};
...
#define sugar_realloc(ptr, size) \
  sugar_realloc_debug(ptr, size, __FILE__, __LINE__)
#define sugar_strdup(str) sugar_strdup_debug(str, __FILE__, __LINE__)
PUB_FUNC void sugar_free_debug(void* ptr);
PUB_FUNC void* sugar_malloc_debug(unsigned long size,
                                  const char* file,
                                  int line);
PUB_FUNC void* sugar_mallocz_debug(unsigned long size,
                                   const char* file,
                                   int line);
PUB_FUNC void* sugar_realloc_debug(void* ptr,
                                   unsigned long size,
                                   const char* file,
                                   int line);
...
ST_FUNC int sugar_add_file_internal(SUGARState* s1,
                                    const char* filename,
                                    int flags);
/* flags: */
...
PUB_FUNC int sugar_parse_args(SUGARState* s,
                              int* argc,
                              char*** argv,
                              int optind);
#ifdef _WIN32
...
ST_FUNC unsigned create_plt_entry(SUGARState* s1,
                                  unsigned got_offset,
                                  struct sym_attr* attr);
ST_FUNC void relocate_plt(SUGARState* s1);
#endif
#endif
ST_FUNC void relocate(SUGARState* s1,
                      ElfW_Rel* rel,
                      int type,
                      unsigned char* ptr,
                      addr_t addr,
...
ST_FUNC int macho_load_dll(SUGARState* s1,
                           int fd,
                           const char* filename,
                           int lev);
#endif
