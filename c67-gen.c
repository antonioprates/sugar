...
#define ALWAYS_ASSERT(x)                                             \
  do {                                                               \
    if (!(x))                                                        \
      sugar_error("internal compiler error file at %s:%d", __FILE__, \
                  __LINE__);                                         \
  } while (0)
...
int C67_map_regn(int r) {
  if (r == 0)       // normal sugar regs
    ...
    return 1;       // B1
  else if (r >= TREG_C67_A4 &&
           r <= TREG_C67_B13)  // these form a pattern of alt pairs
...
void C67_asm(char* s, int a, int b, int c) {
  ...
  if (strstr(s, "MVKL") == s) {
    C67_g((C67_map_regn(b) << 23) | ((a & 0xffff) << 7) | (0x0a << 2) |
          (C67_map_regs(b) << 1));
  } else if (strstr(s, "MVKH") == s) {
    C67_g((C67_map_regn(b) << 23) | (((a >> 16) & 0xffff) << 7) | (0x1a << 2) |
          (C67_map_regs(b) << 1));
  } else if (strstr(s, "STW.D SP POST DEC") == s) {
    C67_g(
        (C67_map_regn(a) << 23) |  // src
        (15 << 18) |               // SP B15
        ...
        (7
         << 4) |  // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU 1=LDBU
        (1 << 2) |                // opcode
    ...
    C67_g((0 << 29) |                // creg
          ...
          (C67_map_regn(c) << 23) |  // dst
          ((C67_map_regn(b) + 1)
           << 18) |    // src2   WEIRD CPU must specify odd reg for some reason
          (0 << 13) |  // src1 NA
          (xpath << 12) |           // x cross path if opposite sides
          (0x1 << 5) |              // opcode
          (0x6 << 2) |              // opcode fixed
...
void C67_B_DISP(int disp)  //  B  +2  Branch with constant displacement
{
  ...
  //
  // we will assume the text section always starts on an 8 word (32 byte
  // boundary)
  //
...
void load(int r, SValue* sv) {
  ...
  if (fr & VT_LVAL) {
    ...
    if ((fr & VT_VALMASK) < VT_CONST)  // check for pure indirect
    {
      ...
      greloc(cur_text_section, sv->sym, ind,
             R_C60LO16);  // rem the inst need to be patched
      ...
      C67_MVKL(C67_A0,
               (fc / element) + 8 / element);  // r=reg to load,  constant
      C67_MVKH(C67_A0,
               (fc / element) + 8 / element);  // r=reg to load,  constant
    ...
    if (v == VT_CONST) {
      ...
      C67_MVKL(
          r, fc + 8);  // r=reg to load, constant C67 stack points to next free
      ...
      t = v & 1;
      C67_B_DISP(4);       //  Branch with constant displacement, skip over this
                           //  branch, load, nop, load
...
void store(int r, SValue* v) {
  ...
  if (bt == VT_LDOUBLE) {
    ...
    if ((v->r & VT_VALMASK) == VT_CONST) {
      ...
      if (v->r & VT_SYM) {
        greloc(cur_text_section, v->sym, ind,
               R_C60LO16);  // rem the inst need to be patched
  ...
  if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
    ...
    if (vtop->r & VT_SYM) {
      ...
      if (is_jmp) {
        ...
        sym = get_sym_ref(&char_pointer_type, cur_text_section, ind + 12,
                          0);  // symbol for return address
        greloc(cur_text_section, sym, ind,
               R_C60LO16);  // rem the inst need to be patched
...
void gfunc_call(int nb_args) {
  ...
  for (i = 0; i < nb_args; i++) {
    if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
      ...
      if (size == 8) {
        C67_STW_PTR_PRE_INC(r + 1, C67_SP,
...
void gfunc_prolog(Sym* func_sym) {
  ...
  for (i = 0; i < NoOfCurFuncArgs; i++) {
    ...
    if (TranslateStackToReg[i] == 8) {
      C67_STW_PTR_PRE_INC(TREG_C67_A4 + i * 2 + 1, C67_SP,
...
void gfunc_epilog(void) {
  {
    int local =
        (-loc + 7) & -8;  // stack must stay aligned to 8 bytes for LDDW instr
    C67_POP(C67_B3);
    C67_NOP(4);                                // NOP wait for load
    C67_IREG_B_REG(0, C67_CREG_ZERO, C67_B3);  //  B.S2  B3
    C67_POP(C67_FP);
    C67_ADDK(local, C67_SP);  //  ADDK.L2 loc,SP
    C67_Adjust_ADDK((int*)(cur_text_section->data + func_sub_sp_offset),
...
ST_FUNC int gjmp_cond(int op, int t) {
  ...
  if (C67_compare_reg !=
          TREG_EAX &&  // check if not already in a conditional test reg
  ...
  C67_IREG_B_REG(C67_invert_test ^ inv, C67_compare_reg,
                 C67_A0);  // [!R] B.S2x  A0
  C67_NOP(5);
  ...
  switch (op) {
    ...
    gen_op8:
      ...
      if (op >= TOK_ULT && op <= TOK_GT)
        gv2(RC_INT_BSIDE,
            RC_INT);  // make sure r (src1) is on the B Side of CPU
      else
        gv2(RC_INT, RC_INT);
    ...
    case TOK_SHL:
      gv2(RC_INT_BSIDE,
          RC_INT_BSIDE);  // shift amount must be on same side as dst
      r = vtop[-1].r;
      fr = vtop[0].r;
      vtop--;
...
void gen_cvt_ftof(int t) {
  ...
  if ((vtop->type.t & VT_BTYPE) == VT_DOUBLE && (t & VT_BTYPE) == VT_FLOAT) {
