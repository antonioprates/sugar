...
ST_DATA const int reg_classes[NB_REGS] = {RC_INT | RC_R(0),
                                          RC_INT | RC_R(1),
                                          RC_INT | RC_R(2),
                                          RC_INT | RC_R(3),
                                          RC_INT | RC_R(4),
...
static void EIu(uint32_t opcode,
                uint32_t func3,
                uint32_t rd,
                uint32_t rs1,
                uint32_t imm) {
...
static void ER(uint32_t opcode,
               uint32_t func3,
               uint32_t rd,
               uint32_t rs1,
               uint32_t rs2,
...
static void EI(uint32_t opcode,
               uint32_t func3,
               uint32_t rd,
               uint32_t rs1,
               uint32_t imm) {
...
static void ES(uint32_t opcode,
               uint32_t func3,
               uint32_t rs1,
               uint32_t rs2,
               uint32_t imm) {
...
ST_FUNC void gsym_addr(int t_, int a_) {
  ...
  while (t) {
    ...
    imm = (((r >> 12) & 0xff) << 12) | (((r >> 11) & 1) << 20) |
          (((r >> 1) & 0x3ff) << 21) | (((r >> 20) & 1) << 31);
  ...
  if (sv->r & VT_SYM) {
    ...
    if (sv->sym->type.t & VT_STATIC) {  // XXX do this per linker relax
      ...
      if (((unsigned)fc + (1 << 11)) >> 12)
        sugar_error("unimp: large addend for global address (0x%llx)",
                    (long long)sv->c.i);
...
ST_FUNC void load(int r, SValue* sv) {
  ...
  if (fr & VT_LVAL) {
    ...
    if (fc != sv->c.i) {
      ...
      if (si != 0) {
        ...
        if (fc < 0)
          pi++;
        o(0x37 | (rr << 7) |
          (((pi + 0x800) & 0xfffff000)));             // lui RR, up(up(fc))
    ...
    if (((unsigned)fc + (1 << 11)) >> 12)
      o(0x37 | (rr << 7) | ((0x800 + fc) & 0xfffff000)),
          rb = rr;  // lui RR, upper(fc)
    ...
    if (is_freg(r) && is_freg(v))
      ER(0x53, 0, rr, freg(v), freg(v),
         bt == VT_DOUBLE ? 0x11
                         : 0x10);  // fsgnj.[sd] RR, V, V == fmv.[sd] RR, V
...
static void reg_pass(CType* type, int* prc, int* fieldofs, int named) {
  ...
  if (prc[0] <= 0 || !named) {
    ...
    fieldofs[1] =
        (0 << 4) |
        (size <= 1 ? VT_BYTE
                   : size <= 2 ? VT_SHORT : size <= 4 ? VT_INT : VT_LLONG);
    fieldofs[2] =
        (8 << 4) |
        (size <= 9 ? VT_BYTE
                   : size <= 10 ? VT_SHORT : size <= 12 ? VT_INT : VT_LLONG);
  }
}
...
ST_FUNC void gfunc_prolog(Sym* func_sym) {
  ...
  while ((sym = sym->next) != NULL) {
    ...
    if (areg[prc[1] - 1] >= 8 ||
      ...
      for (i = 0; i < regcount; i++) {
        if (areg[prc[1 + i] - 1] >= 8) {
          ...
          ES(0x27, (size / regcount) == 4 ? 2 : 3, 8, 10 + areg[1]++,
             loc + (fieldofs[i + 1] >> 4));  // fs[wd] FAi, loc(s0)
        } else {
    ...
    sym_push(sym->v & ~SYM_FIELD, &sym->type,
             (byref ? VT_LLOCAL : VT_LOCAL) | VT_LVAL, param_addr);
  }
...
ST_FUNC void gen_opf(int op) {
  ...
  switch (op) {
    ...
    arithop:
      ...
      vtop->r = rd;
      rd = freg(rd);
      ER(0x53, 7, rd, rs1, rs2,
         dbl | (op << 2));  // fop.[sd] RD, RS1, RS2 (dyn rm)
      break;
    case '-':
      op = 1;  // fsub
      goto arithop;
    ...
    cmpop:
      ...
      vtop->r = rd;
      rd = ireg(rd);
      ER(0x53, op, rd, rs1, rs2,
         dbl | 0x50);  // fcmp.[sd] RD, RS1, RS2 (op == eq/lt/le)
      if (invert)
...
ST_FUNC void gen_cvt_ftoi(int t) {
  ...
  int u = t & VT_UNSIGNED;
  if (ft == VT_LDOUBLE) {
    int func = l ? (u ? TOK___fixunstfdi : TOK___fixtfdi)
