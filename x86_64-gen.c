...
ST_DATA const int reg_classes[NB_REGS] = {
    /* eax */ RC_INT | RC_RAX,
    /* ecx */ RC_INT | RC_RCX,
    /* edx */ RC_INT | RC_RDX, 0, 0, 0, 0, 0, RC_R8, RC_R9, RC_R10, RC_R11, 0,
    0, 0, 0,
...
static int is64_type(int t) {
  return ((t & VT_BTYPE) == VT_PTR || (t & VT_BTYPE) == VT_FUNC ||
          (t & VT_BTYPE) == VT_LLONG);
}
...
static void gen_gotpcrel(int r, Sym* sym, int c) {
  ...
  sugar_error("internal error: no GOT on PE: %s %x %x | %02x %02x %02x\n",
              get_tok_str(sym->v, NULL), c, r, cur_text_section->data[ind - 3],
              cur_text_section->data[ind - 2], cur_text_section->data[ind - 1]);
#endif
...
void load(int r, SValue* sv) {
  ...
  if ((fr & VT_VALMASK) == VT_CONST && (fr & VT_SYM) && (fr & VT_LVAL) &&
  ...
  if (fr & VT_LVAL) {
    ...
    if ((ft & VT_BTYPE) == VT_FLOAT) {
      ...
      assert(((ft & VT_BTYPE) == VT_INT) || ((ft & VT_BTYPE) == VT_LLONG) ||
             ((ft & VT_BTYPE) == VT_PTR) || ((ft & VT_BTYPE) == VT_FUNC));
      ll = is64_type(ft);
      b = 0x8b;
    }
    if (ll) {
...
void gfunc_call(int nb_args) {
  ...
  for (i = 0; i < nb_args; i++) {
    ...
    if (using_regs(size))
      continue; /* arguments smaller than 8 bytes passed in registers or on
                   stack */
    ...
    if (!using_regs(size)) {
      ...
      if (is_sse_float(vtop->type.t)) {
        ...
        if (bt == VT_STRUCT) {
          ...
          vtop->type.t =
              size > 4 ? VT_LLONG
                       : size > 2 ? VT_INT : size > 1 ? VT_SHORT : VT_BYTE;
        }
...
static X86_64_Mode classify_x86_64_arg(CType* ty,
                                       CType* ret,
                                       int* psize,
                                       int* palign,
                                       int* reg_count) {
...
ST_FUNC int gfunc_sret(CType* vt,
  ...
  *ret_align = 1;  // Never have to re-align return values for x86-64
  *regsize = 8;
  return (classify_x86_64_arg(vt, ret, &size, &align, &reg_count) !=
          x86_64_mode_memory);
}
...
void gfunc_call(int nb_args) {
  ...
  if (vtop->type.ref->f.func_type !=
      FUNC_NEW) /* implies FUNC_OLD or FUNC_ELLIPSIS */
...
void gen_cvt_itof(int t) {
  if ((t & VT_BTYPE) == VT_LDOUBLE) {
    ...
    if ((vtop->type.t & (VT_BTYPE | VT_UNSIGNED)) == (VT_INT | VT_UNSIGNED) ||
...
ST_FUNC void gen_cvt_csti(int t) {
  ...
  orex(ll, r, 0,
       0xc0b60f /* mov[sz] %a[xl], %eax */
           | (sz << 3 | xl) << 8 | (REG_VALUE(r) << 3 | REG_VALUE(r)) << 16);
}
