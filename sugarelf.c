  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if (sr->sh_type == SHT_RELX && sr->link == s) {
      ...
      for (; rel < rel_end; ++rel) {
        ...
        // if (n < 0) sugar_error("internal: invalid symbol index in
        // relocation");
...
ST_FUNC Section* new_section(SUGARState* s1,
                             const char* name,
                             int sh_type,
                             int sh_flags) {
  Section* sec;
...
static Section* find_section_create(SUGARState* s1,
                                    const char* name,
                                    int create) {
  Section* sec;
  int i;
...
/* return elf symbol value, signal error if 'err' is nonzero, decorate
   name if FORC */
ST_FUNC addr_t get_sym_addr(SUGARState* s1,
                            const char* name,
                            int err,
                            int forc) {
  int sym_index;
...
ST_FUNC void list_elf_symbols(SUGARState* s,
                              void* ctx,
                              void (*symbol_cb)(void* ctx,
                                                const char* name,
                                                const void* val)) {
  ElfW(Sym) * sym;
  Section* symtab;
...
/* list elf symbol names and values */
LIBSUGARAPI void sugar_list_symbols(SUGARState* s,
                                    void* ctx,
                                    void (*symbol_cb)(void* ctx,
                                                      const char* name,
                                                      const void* val)) {
...
#ifndef ELF_OBJ_ONLY
static void version_add(SUGARState* s1) {
  int i;
  ElfW(Sym) * sym;
...
ST_FUNC int set_elf_sym(Section* s,
                        addr_t value,
                        unsigned long size,
                        int info,
                        int other,
...
/* put relocation */
ST_FUNC void put_elf_reloca(Section* symtab,
                            Section* s,
...
ST_FUNC void put_elf_reloc(Section* symtab,
                           Section* s,
                           unsigned long offset,
                           int type,
                           int symbol) {
...
ST_FUNC void squeeze_multi_relocs(Section* s, size_t oldrelocoffset) {
  ...
  for (a = oldrelocoffset + sizeof(*r); a < sr->data_offset; a += sizeof(*r)) {
    ...
    for (; i >= (ssize_t)oldrelocoffset &&
           ((ElfW_Rel*)(sr->data + i))->r_offset > addr;
         i -= sizeof(*r)) {
...
ST_FUNC void put_stabs(SUGARState* s1,
                       const char* str,
                       int type,
                       int other,
                       int desc,
...
ST_FUNC void put_stabs_r(SUGARState* s1,
                         const char* str,
                         int type,
                         int other,
                         int desc,
...
static int prepare_dynamic_rel(SUGARState* s1, Section* sr) {
  ...
  return count;
    ...
    (defined(SUGAR_TARGET_MACHO) && defined SUGAR_IS_NATIVE)
static void build_got(SUGARState* s1) {
  /* if no got, then create it */
  s1->got = new_section(s1, ".got", SHT_PROGBITS, SHF_ALLOC | SHF_WRITE);
  s1->got->sh_entsize = 4;
  set_elf_sym(symtab_section, 0, 4, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
...
static struct sym_attr* put_got_entry(SUGARState* s1,
                                      int dyn_reloc_type,
                                      int sym_index) {
  int need_plt_entry;
  const char* name;
...
ST_FUNC int set_global_sym(SUGARState* s1,
                           const char* name,
                           Section* sec,
                           addr_t offs) {
...
static void set_local_sym(SUGARState* s1,
                          const char* name,
                          Section* s,
                          int offset) {
...
static void sugar_add_linker_symbols(SUGARState* s1) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if ((s->sh_flags & SHF_ALLOC) &&
        (s->sh_type == SHT_PROGBITS || s->sh_type == SHT_STRTAB)) {
      const char* p;
...
static void sugar_output_binary(SUGARState* s1, FILE* f, const int* sec_order) {
  Section* s;
  int i, offset, size;
...
static int alloc_sec_names(SUGARState* s1, int file_type, Section* strsec) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    /* when generating a DLL, we include relocations but we may
       patch them */
#ifndef ELF_OBJ_ONLY
    if (file_type == SUGAR_OUTPUT_DLL && s->sh_type == SHT_RELX &&
      ...
      if (!(s1->sections[s->sh_info]->sh_flags & SHF_WRITE))
        ...
        if ((s1->do_debug && s->sh_type != SHT_RELX) ||
            file_type == SUGAR_OUTPUT_OBJ || (s->sh_flags & SHF_ALLOC) ||
            i == (s1->nb_sections - 1)
...
static int layout_sections(SUGARState* s1,
                           ElfW(Phdr) * phdr,
                           int phnum,
                           Section* interp,
                           Section* strsec,
    ...
    for (j = 0; j < (phnum == 6 ? 3 : 2); j++) {
      ...
      for (k = 0; k < 5; k++) {
        for (i = 1; i < s1->nb_sections; i++) {
          ...
          if (j == 0) {
            if ((s->sh_flags & (SHF_ALLOC | SHF_WRITE | SHF_TLS)) != SHF_ALLOC)
              continue;
          } else if (j == 1) {
...
static void fill_unloadable_phdr(ElfW(Phdr) * phdr,
                                 int phnum,
                                 Section* interp,
                                 Section* dynamic) {
  ElfW(Phdr) * ph;
...
static void sugar_output_elf(SUGARState* s1,
                             FILE* f,
                             int phnum,
                             ElfW(Phdr) * phdr,
                             int file_offset,
...
static int sugar_write_elf_file(SUGARState* s1,
                                const char* filename,
                                int phnum,
                                ElfW(Phdr) * phdr,
                                int file_offset,
...
static void tidy_section_headers(SUGARState* s1, int* sec_order) {
  ...
  for_each_elem(symtab_section, 1, sym,
                ElfW(Sym)) if (sym->st_shndx != SHN_UNDEF &&
                               sym->st_shndx < SHN_LORESERVE) sym->st_shndx =
      backmap[sym->st_shndx];
  if (!s1->static_link) {
    for_each_elem(s1->dynsym, 1, sym,
                  ElfW(Sym)) if (sym->st_shndx != SHN_UNDEF &&
                                 sym->st_shndx < SHN_LORESERVE) sym->st_shndx =
        backmap[sym->st_shndx];
  }
...
static int elf_output_file(SUGARState* s1, const char* filename) {
  ...
  if (file_type == SUGAR_OUTPUT_OBJ)
    ...
    int i;
    for (i = 1; i < s1->nb_sections && !(s1->sections[i]->sh_flags & SHF_TLS);
         i++)
      ;
  ...
  file_offset =
      layout_sections(s1, phdr, phnum, interp, strsec, &dyninf, sec_order);
...
ST_FUNC int sugar_load_object_file(SUGARState* s1,
  ...
  if (ehdr.e_ident[5] != ELFDATA2LSB || ehdr.e_machine != EM_SUGAR_TARGET) {
  fail1:
    ...
    switch (s->sh_type) {
      case SHT_RELX:
        ...
        for_each_elem(s, (offset / sizeof(*rel)), rel, ElfW_Rel) {
          ...
          invalid_reloc:
            sugar_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x", i,
...
static int sugar_load_alacarte(SUGARState* s1,
                               int fd,
                               int size,
                               int entrysize) {
...
static void set_ver_to_ver(SUGARState* s1,
                           int* n,
                           int** lv,
                           int i,
                           char* lib,
  ...
  if ((*lv)[i] == -1) {
    ...
    if (v == nb_sym_versions) {
      sym_versions =
          sugar_realloc(sym_versions, (v + 1) * sizeof(*sym_versions));
...
/* Associates symbol SYM_INDEX (in dynsymtab) with sym-version index
   VERNDX.  */
static void set_sym_version(SUGARState* s1, int sym_index, int verndx) {
  if (sym_index >= nb_sym_to_version) {
    int newelems = sym_index ? sym_index * 2 : 1;
    sym_to_version =
        sugar_realloc(sym_to_version, newelems * sizeof(*sym_to_version));
...
ST_FUNC int sugar_load_dll(SUGARState* s1,
                           int fd,
                           const char* filename,
                           int level) {
  ElfW(Ehdr) ehdr;
