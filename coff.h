...
#define ISINT(x)                                                             \
  (((x) >= T_CHAR && (x) <= T_LONG) || ((x) >= T_UCHAR && (x) <= T_ULONG) || \
   (x) == T_ENUM)
...
#define INCREF_COFF(x)                                                     \
  ((((x) & ~N_BTMASK_COFF) << N_TSHIFT_COFF) | (DT_PTR << N_BTSHFT_COFF) | \
   (x & N_BTMASK_COFF))
#define DECREF_COFF(x) \
  ((((x) >> N_TSHIFT_COFF) & ~N_BTMASK_COFF) | ((x)&N_BTMASK_COFF))
